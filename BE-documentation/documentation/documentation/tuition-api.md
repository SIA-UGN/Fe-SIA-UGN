...
- [A. Endpoint Mahasiswa](#a-endpoint-mahasiswa)
  ...
  - [A3. Pembayaran (Manual)](#a3-pembayaran)
  - [A4. Pembayaran Midtrans (Virtual Account)](#a4-pembayaran-midtrans-virtual-account)
...
- [F. Integrasi Midtrans](#f-integrasi-midtrans)
...
| 4 | POST | `/api/student/tuition/{id}/pay` | Mahasiswa | Upload bukti pembayaran (manual) |
...
| **7** | **POST** | **`/api/student/tuition/{id}/checkout`** | **Mahasiswa** | **Checkout VA Midtrans** |
| **8** | **GET** | **`/api/student/tuition/{id}/payment-status`** | **Mahasiswa** | **Cek status pembayaran Midtrans** |
| 9 | GET | `/api/admin/tuition/dashboard` | Admin/Manager | Dashboard statistik |
| 10 | GET | `/api/admin/tuition/rates` | Admin/Manager | Daftar tarif UKT |
| 11 | POST | `/api/admin/tuition/rates` | Admin/Manager | Buat tarif UKT |
| 12 | PUT | `/api/admin/tuition/rates/{id}` | Admin/Manager | Update tarif UKT |
| 13 | DELETE | `/api/admin/tuition/rates/{id}` | Admin/Manager | Hapus tarif UKT |
| 14 | GET | `/api/admin/tuition/bills` | Admin/Manager | Daftar tagihan |
| 15 | POST | `/api/admin/tuition/bills` | Admin/Manager | Buat tagihan individu |
| 16 | POST | `/api/admin/tuition/bills/generate` | Admin/Manager | Generate tagihan massal |
| 17 | GET | `/api/admin/tuition/bills/{id}` | Admin/Manager | Detail tagihan |
| 18 | PUT | `/api/admin/tuition/bills/{id}` | Admin/Manager | Update tagihan |
| 19 | GET | `/api/admin/tuition/payments` | Admin/Manager | Daftar pembayaran |
| 20 | GET | `/api/admin/tuition/payments/{id}` | Admin/Manager | Detail pembayaran |
| 21 | PATCH | `/api/admin/tuition/payments/{id}/verify` | Admin/Manager | Verifikasi pembayaran |
| 22 | PATCH | `/api/admin/tuition/payments/{id}/reject` | Admin/Manager | Tolak pembayaran |
| 23 | GET | `/api/admin/tuition/virtual-accounts` | Admin/Manager | Daftar Virtual Account |
| 24 | POST | `/api/admin/tuition/virtual-accounts/generate` | Admin/Manager | Generate VA massal |
| **25** | **POST** | **`/api/midtrans/webhook`** | **Publik** | **Webhook notifikasi Midtrans** |

---

## F. Integrasi Midtrans

Modul pembayaran UKT sekarang mendukung pembayaran otomatis via **Midtrans Virtual Account**. Dengan integrasi ini, mahasiswa dapat membayar UKT melalui ATM/m-banking/internet banking dan status pembayaran akan diupdate secara otomatis tanpa perlu verifikasi manual admin.

**Dokumentasi lengkap Midtrans:** Lihat [midtrans-api.md](midtrans-api.md)

### Metode Pembayaran

| Metode | Flow | Verifikasi |
|--------|------|------------|
| **Midtrans VA** (baru) | Checkout → bayar via ATM → webhook otomatis | ✅ Otomatis via webhook |
| **Manual** (existing) | Upload bukti bayar → admin review | 👤 Manual oleh admin |

### Endpoint Baru (Midtrans)

| Method | Endpoint | Deskripsi |
|--------|----------|-----------|
| `POST` | `/api/student/tuition/{id}/checkout` | Buat transaksi VA Midtrans |
| `GET` | `/api/student/tuition/{id}/payment-status` | Cek status pembayaran real-time |
| `POST` | `/api/midtrans/webhook` | Webhook notifikasi dari Midtrans |
