...
import { Check, X, RotateCcw, Loader2, AlertCircle, Award, FileText } from 'lucide-react';
import Navbar from '@/components/ui/navigation-menu';
import Footer from '@/components/ui/footer';
import { getPengabdianManager, validasiPengabdian } from '@/lib/managerApi';
...
export default function ReviewPengabdianPage() {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const res = await getPengabdianManager();
            const found = (res?.data ?? []).find(p => String(p.id) === String(id));
            if (found) setItem(found);
            else setError('Pengabdian tidak ditemukan atau sudah selesai divalidasi.');
        } catch (err) {
    ...
    const ketua = authors.find(a => a.pivot?.peran === 'Ketua') ?? authors[0];
    ...
    const submit = async (status_validasi, body) => {
        setActing(true);
        try {
            await validasiPengabdian(id, { status_validasi, ...body });
            backToAktivitas();
        } catch (err) {
    ...
    const subtitle = item ? [item.jenis_pkm, item.tahun_pelaksanaan && `Tahun ${item.tahun_pelaksanaan}`].filter(Boolean).join('  ·  ') : '';
    ...
    return (
        <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#E6EEE9', fontFamily: 'Urbanist, sans-serif' }}>
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <div className="bg-white rounded-[24px] p-7 shadow-sm mb-6">
                            <h2 className="text-lg font-bold mb-4" style={{ color: '#1a4731' }}>Pelaksana ({authors.length} orang)</h2>
                            <div className="flex flex-col gap-3">
                                ...
                                {authors.length === 0 && <p className="text-sm text-gray-400">Tidak ada pelaksana.</p>}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[24px] p-7 shadow-sm mb-6">
                            ...
                            <div className="flex items-center gap-4 rounded-2xl border px-4 py-3" style={{ borderColor: '#E5E7EB' }}>
                                ...
                                <div className="min-w-0 flex-1">
                                    <p className="text-[15px] font-bold text-gray-800 truncate">{item.file_laporan ? String(item.file_laporan).split('/').pop() : 'Proposal PKM'}</p>
                                    <p className="text-xs text-gray-400">Proposal PKM</p>
                                </div>
                                {item.file_laporan ? (
                                    <a href={fileUrl(item.file_laporan)} target="_blank" rel="noreferrer" className="text-xs font-bold px-3 py-1.5 rounded-full shrink-0" style={{ backgroundColor: '#E5E7EB', color: '#6B7280' }}>PDF</a>
                                ) : (
            ...
            {modal === 'setujui' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onClick={() => setModal(null)}>
                    <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-extrabold text-gray-800 mb-1">Setujui Pengabdian</h3>
                        <p className="text-sm text-gray-500 mb-4">Tetapkan Angka Kredit (AK). Kegiatan akan berstatus <strong>Aktif</strong>.</p>
                        ...
                        <input type="number" min="0" value={ak} onChange={e => setAk(e.target.value)} placeholder="mis. 15"
            ...
            {(modal === 'tolak' || modal === 'revisi') && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onClick={() => setModal(null)}>
                    <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-extrabold text-gray-800 mb-1">{modal === 'revisi' ? 'Minta Revisi' : 'Tolak Pengabdian'}</h3>
