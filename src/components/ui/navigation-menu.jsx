...
import { User, UserCog, LogOut, Menu, X, ChevronDown } from 'lucide-react'
...
const NavbarDropdownItem = forwardRef(({ className, label, items, ...props }, ref) => {
  ...
  return (
    ...
    >
      <button
        className={cn(
          "flex items-center gap-1 text-white hover:text-brand-yellow transition-colors duration-200 font-medium text-sm lg:text-base relative pb-1",
          className
        )}
        style={{ color: isActive ? '#DABC4E' : '' }}
      >
        {label}
        <ChevronDown
          size={14}
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-2 px-4 py-3 text-sm font-medium transition-colors duration-150 hover:bg-gray-50"
                style={{ color: itemActive ? '#015023' : '#374151', backgroundColor: itemActive ? '#E6EEE9' : '' }}
              >
                <span
                  className="w-2 h-2 rounded-full shrink-0"
                  style={{ backgroundColor: itemActive ? '#015023' : '#DABC4E' }}
                />
                {item.label}
              </Link>
            );
  ...
  <>
    ...
    >
      ...
      <NavbarDropdownItem
        label="Kehadiran"
        items={[
          { href: '/kehadiran', label: 'Kehadiran Dosen' },
          { href: '/kehadiran-mahasiswa', label: 'Kehadiran Mahasiswa' },
        ]}
      />
    ...
    >
      ...
      <div className="flex flex-col p-4 gap-2">
        ...
        <div className="flex flex-col">
          <p className="px-4 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider" style={{ color: '#9ca3af' }}>
            Kehadiran
          </p>
          <MobileNavMenuItem
            href="/kehadiran"
